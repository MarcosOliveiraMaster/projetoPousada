...
import { BIBLIOTECA_ITENS } from '../../constants/itensBiblioteca'
...
export default function ModalBibliotecaItens({ onSelecionar, onFechar }: Props) {
  const [busca, setBusca] = useState('')
  const filtrados = BIBLIOTECA_ITENS.filter(i => i.label.toLowerCase().includes(busca.toLowerCase()))

  return (
    <Modal titulo="Biblioteca de itens" onFechar={onFechar} largura="max-w-xl">
        ...
        {filtrados.map(item => {
          ...
          return (
            <button key={item.value} onClick={() => onSelecionar(item.value, item.label)}
              className="flex flex-col items-center gap-1.5 p-3 rounded-xl border-2 border-sand-200 text-sand-500 hover:border-brand-400 hover:text-brand-700 hover:bg-brand-50 transition-all">
